...
def top_gainers_today(limit: int = 10) -> pd.DataFrame:
    ...
    1) Try Polygon snapshot gainers (tickers + % change so far).
    2) If Polygon not available, fallback to Alpha Vantage TOP_GAINERS_LOSERS.
    Returns DataFrame: [symbol, price, change_pct, source]
    """
    items = []
    # 1) Polygon snapshot gainers (preferred)
    if POLY_KEY:
        try:
    ...
    # 2) Fallback: Alpha Vantage top gainers
    if not items:
        if not AV_KEY:
...
def rank_candidates_with_model(
    ...
    download_hist_fn: Callable[[str, int], "pd.DataFrame"],
    predictor_fn: Callable[..., dict],
    horizon_days: int = 7
) -> pd.DataFrame:
    """Run your predictor model on each candidate to estimate expected returns & prob_up.
    Expects:
      - download_hist_fn(symbol, days) -> DataFrame with Date + Close
      - predictor_fn(df[['Close']], horizon_days=?, price_col='Close') -> {'ok', 'expected_return_pct','prob_up','score'}
    Returns a ranked DataFrame by expected_return_pct then prob_up.
    """
    rows = []
    symbols = list(gainers_df["symbol"]) if not gainers_df.empty else []
    for sym in symbols:
        try:
    ...
    if out.empty:
        return out
    out = out.sort_values(["expected_return_pct","prob_up"], ascending=[False, False]).reset_index(drop=True)
    # reattach price/today change for display
    if not gainers_df.empty:
        out = out.merge(gainers_df[["symbol","price","change_pct","source"]], on="symbol", how="left")
    return out

def handle_highest_riser_intent(download_hist_fn, predictor_fn, limit: int = 10, horizon_days: int = 7) -> str:
    """Return a formatted message for chat:
       - Fetch top gainers (Polygon -> AV fallback)
       - Score with model
       - Rank and present
       Message format:
         Model pick: XYZ
    ...
    ranked = rank_candidates_with_model(gainers, download_hist_fn, predictor_fn, horizon_days=horizon_days)
    if ranked is None or ranked.empty:
        # Show raw gainers as last resort
        lines = [f"**{r.symbol}** {float(r.change_pct or 0):+.2f}% (price {r.price})" for r in gainers.head(5).itertuples()]
    ...
    reply = [
        ...
        f"- Today’s gain: {float(top_pick.get('change_pct') or 0):+.2f}%  |  Price: {top_pick.get('price')}  |  Source: {top_pick.get('source')}",
        "",
        "Other candidates:",
    ]
